...
def create_campaign(request):
    ...
    templates = template_Colection.find().sort("created_at", -1)
...
def all_templates(request):
    # all_template = Templates.objects.all()
    all_template = template_Colection.find()
    return render(request,"demosendy/all_templates.html",{"templates":all_template})
    ...
    if request.method == "POST":
        ...
        if template_name:
            ...
            try:
                ...
                template_Colection.insert_one(template_payload)
                print("inserting the values")
                # print(template_payload)
                # template_data = Templates(template_name=template_name,template_Content=template_content)
                # template_data.save()
